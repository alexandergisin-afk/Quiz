...
async function fetchLeaderboard() {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/scores?select=name,score,topic,level,created_at&order=score.desc&limit=20`,
...
async function submitScore(name, score, topic, level) {
  ...
  if (existing !== null) {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/scores?name=eq.${encodeURIComponent(cleanName)}`, {
      ...
      body: JSON.stringify({ score, topic, level }),
    });
  ...
  const res = await fetch(`${SUPABASE_URL}/rest/v1/scores`, {
    ...
    body: JSON.stringify({ name: cleanName, score, topic, level }),
  });
  if (!res.ok) throw new Error("Eintrag konnte nicht gespeichert werden.");
  return { skipped: false };
}
...
const WHATS_NEW = [
  ...
  { icon: "🗂️", title: "Aufgeräumtes Menü", text: "Klarere Modus-Auswahl; Zeit & Schwierigkeit beim Solo-Start." },
];
...
export default function LaenderDuell() {
  ...
  const [hideFinishedDuels, setHideFinishedDuels] = useState(false);
  ...
  // Duell, Online & Iron Man: knackige 10s. Sonst die gewählte Zeit.
  const roundSeconds = (gameMode === "duel" || gameMode === "online" || gameMode === "ironman") ? 10
    : timeModeMeta.seconds; // 0 = no timer
  ...
  const startDuel = () => {
    setSelectedTopic("mix");
    setGameMode("duel");
    setQuestions(buildMixRound(TOTAL_QUESTIONS, "schwer")); // Duell ist immer knackig: nur schwere Fragen
    resetRoundState();
    setScreen("game");
  };

  // Iron Man — endlos, bis zum ersten Fehler. Knackig: nur schwere Fragen.
  const startIronman = () => {
    setSelectedTopic("mix");
    setGameMode("ironman");
    setQuestions(buildMixRound(IRONMAN_POOL, "schwer"));
    resetRoundState();
    setScreen("game");
  };
  ...
  const challengePlayer = async (opponent) => {
    ...
    setOdError("");
    try {
      const qs = buildMixRound(TOTAL_QUESTIONS, "schwer"); // Online-Duell ebenfalls knackig
  ...
  const handleSubmitScore = async () => {
    ...
    try {
      ...
      if (gameMode === "daily") {
        ...
        result = await submitScore(playerName.trim(), points, topicMeta.label, level);
        setSubmitted(true);
      }
    ...
    <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              ...
              <div style={{ textAlign: "left", flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: "#94c8d8" }}>
                  ⏱️ 10 Sek · 🔴 Schwer · bis zum ersten Fehler
      ...
      {screen === "topics" && (
        <div style={{ textAlign: "center", maxWidth: 440, width: "100%" }}>
          ...
          <p style={{ color: "#94c8d8", fontSize: 14, margin: "0 0 20px" }}>Worüber willst du raten?</p>

          {/* Zeit-Wähler */}
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 12, color: "#7aa8b8", marginBottom: 7, textAlign: "left", fontWeight: 600 }}>⏱️ Zeit pro Frage</div>
              ...
              {TIME_MODES.map(m => {
                ...
                return (
                  <button key={m.id} onClick={() => setTimeMode(m.id)} style={{
                    ...
                    <div style={{ fontSize: 16, lineHeight: 1, marginBottom: 3 }}>{m.icon}</div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: active ? "#e8f4f8" : "#94c8d8" }}>{m.label}</div>
                  </button>
                );
              })}
            </div>
          ...
          <div style={{ marginBottom: 22 }}>
            <div style={{ fontSize: 12, color: "#7aa8b8", marginBottom: 7, textAlign: "left", fontWeight: 600 }}>🎯 Schwierigkeit</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 7 }}>
              {DIFFICULTY_LEVELS.map(lvl => {
                ...
                return (
                  <button key={lvl.id} onClick={() => setDifficulty(lvl.id)} style={{
                    ...
                    <div style={{ fontSize: 15, lineHeight: 1, marginBottom: 3 }}>{lvl.icon}</div>
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <div style={{ flex: 1, textAlign: "left", minWidth: 0 }}>
                        <div style={{ fontSize: 15, fontWeight: 600, color: "#e8f4f8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 6 }}>
                          <span style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{e.name}</span>
                          {lbTab === "solo" && e.level > 0 && (
                            <span style={{
                              flexShrink: 0, fontSize: 10, fontWeight: 700, color: "#c4b5fd",
        ...
        <div style={{ textAlign: "center", maxWidth: 460, width: "100%" }}>
          ...
          {!LEADERBOARD_ENABLED ? (
            ...
            <>
              ...
              {/* Schalter: abgeschlossene ausblenden */}
              {onlineDuels.length > 0 && (() => {
                const me = playerName.trim();
                const finishedCount = onlineDuels.filter(d => (d.p1_done && d.p2_done)).length;
                if (finishedCount === 0) return null;
                return (
                  <button onClick={() => setHideFinishedDuels(v => !v)} style={{
                    display: "block", marginLeft: "auto", marginBottom: 10,
                    background: "none", border: "none", color: "#64d8ff",
                    fontSize: 12, cursor: "pointer", textDecoration: "underline",
                  }}>
                    {hideFinishedDuels ? `Abgeschlossene zeigen (${finishedCount})` : "Abgeschlossene ausblenden"}
                  </button>
                );
              })()}
              ...
              {odLoading ? (
                ...
                const me = playerName.trim();
                // Phase: 0 = du bist dran, 1 = wartet auf Gegner, 2 = abgeschlossen
                const phaseOf = (d) => {
                  const isP1 = d.p1 === me;
                ...
                let list = onlineDuels.slice();
                if (hideFinishedDuels) list = list.filter(d => phaseOf(d) !== 2);
                list.sort((a, b) => {
                  const pa = phaseOf(a), pb = phaseOf(b);
                  if (pa !== pb) return pa - pb; // dran → wartet → fertig
                  return (b.id || 0) - (a.id || 0); // innerhalb gleicher Phase: neueste zuerst
                });
                if (list.length === 0) {
                  return (
                    <div style={{ padding: "30px 20px", color: "#7aa8b8", fontSize: 14 }}>
                      Keine offenen Duelle. 🎉
                    </div>
                  );
                }
                return (
